...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        $stmt = $pdo->prepare("INSERT INTO vprasanja (vprasanje, tip, odgovori, pravilni_odgovor, vrstni_red, aktivno)
                                VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->execute([$vprasanje, $tip, $odgovoriJson, $pravilniJson, $vrstniRed, $aktivno]);
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-lg-10 py-4">
                <h2 class="mb-4" style="font-family: var(--font-display);">Dodaj vprašanje</h2>
                ...
                <div class="admin-card">
                    <form method="POST" id="questionForm">
                        <div class="mb-3">
                            ...
                            <textarea name="vprasanje" class="form-control" rows="2" required><?= htmlspecialchars($_POST['vprasanje'] ?? '') ?></textarea>
                        </div>
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-4">
                                ...
                                <select name="tip" id="tip" class="form-select">
                                    <option value="select">Spustni seznam (en odgovor)</option>
                                    <option value="radio">Enoizbirno (radio)</option>
                                    <option value="checkbox">Večizbirno (checkbox)</option>
                                    <option value="text">Prosto besedilo</option>
                                </select>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label fw-semibold">Vrstni red</label>
                                <input type="number" name="vrstni_red" class="form-control" value="<?= (int) ($_POST['vrstni_red'] ?? 0) ?>">
                            </div>
                            <div class="col-md-4 d-flex align-items-end">
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input" name="aktivno" id="aktivno" checked>
                        ...
                        <div id="textAnswerWrap" class="mb-3 d-none">
                            ...
                            <input type="text" name="text_answer" class="form-control" value="<?= htmlspecialchars($_POST['text_answer'] ?? '') ?>">
                        </div>

                        <button type="submit" class="btn btn-bronze"><i class="fa-solid fa-floppy-disk me-1"></i>Shrani vprašanje</button>
